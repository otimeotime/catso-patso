...
namespace {
    ...
    static vector<string> frozenlake_map() {
        return {
            "SFFF",
            "FHFH",
            "FFFH",
    ...
    static vector<Candidate> build_candidates() {
        ...
        cands.push_back({"MENTS", "temp=0.01,epsilon=0.75", [](auto env, auto init_state, int max_depth, int seed) {
        ...
        cands.push_back({"RENTS", "temp=0.01,epsilon=0.75", [](auto env, auto init_state, int max_depth, int seed) {
            mcts::MentsManagerArgs args(env);
            args.max_depth = max_depth;
            args.mcts_mode = false;
            args.temp = 0.01;
        ...
        cands.push_back({"TENTS", "temp=0.05,epsilon=0.75", [](auto env, auto init_state, int max_depth, int seed) {
            mcts::MentsManagerArgs args(env);
            args.max_depth = max_depth;
            args.mcts_mode = false;
            args.temp = 0.05;
        ...
        cands.push_back({"DENTS", "temp=0.05,epsilon=0.75,use_dp_value=true", [](auto env, auto init_state, int max_depth, int seed) {
        ...
        cands.push_back({"BTS", "temp=0.05,epsilon=0.5", [](auto env, auto init_state, int max_depth, int seed) {
            mcts::DentsManagerArgs args(env);
            args.max_depth = max_depth;
            args.mcts_mode = false;
            args.temp = 0.05;
        ...
        cands.push_back({"VarDE", "temp=0.05,variance_floor=0.01,use_dp_value=true", [](auto env, auto init_state, int max_depth, int seed) {
            mcts::RvipManagerArgs args(env);
            args.max_depth = max_depth;
            args.mcts_mode = false;
            args.temp = 0.05;
            args.variance_floor = 0.01;
            args.seed = seed;
            args.use_dp_value = true;
...
int main(int argc, char** argv) {
    ...
    auto env = make_shared<mcts::exp::FrozenLakeEnv>(frozenlake_map(), horizon, gamma, p);
    auto init_state = env->get_initial_state_itfc();
    auto candidates = build_candidates();

    ofstream out("results_frozenlake.csv", ios::out | ios::trunc);
    out << "env,algorithm,run,trial,mc_mean,mc_stddev\n";

    cout << "[exp] FrozenLake, horizon=" << horizon << ", gamma=" << gamma << ", p=" << p << "\n";
    ...
    cout << "\nResults written to results_frozenlake.csv\n";
    return 0;
}
