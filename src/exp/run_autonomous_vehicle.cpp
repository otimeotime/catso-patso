...
namespace {
    ...
    static vector<Candidate> build_candidates(double cvar_tau) {
        ...
        cands.push_back({"CATSO", "n_atoms=51,optimism=1.0,p=1.0,tau=" + to_string(cvar_tau), [cvar_tau](auto env, auto init_state, int max_depth, int seed) {
        ...
        cands.push_back({"PATSO", "max_particles=64,optimism=1.0,p=1.0,tau=" + to_string(cvar_tau), [cvar_tau](auto env, auto init_state, int max_depth, int seed) {
...
int main(int argc, char** argv) {
    ...
    AutonomousVehicleCvarOracle oracle(env, cvar_tau);
    ...
    for (const auto& [algo_trial, summary] : summary_by_algo_and_trial) {
        ...
        summary_out << "autonomous_vehicle," << algo << "," << trial
    ...
    cout << "\nResults written to results_autonomous_vehicle.csv and results_autonomous_vehicle_summary.csv\n";
    return 0;
}
