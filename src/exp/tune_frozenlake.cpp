...
namespace {
    ...
    static vector<Candidate> build_candidates() {
        ...
        for (double temp : {0.01, 0.05, 0.1, 1.0}) {
        ...
        for (double temp : {0.01, 0.05, 0.1, 1.0}) {
            for (double variance_floor : {0.0001, 0.001, 0.01, 0.1}) {
...
int main(int argc, char** argv) {
    ...
    ofstream out("tune_frozenlake.csv", ios::out | ios::trunc);
    ...
    cout << "[tune] FrozenLake, horizon=" << horizon << ", trials=" << total_trials << ", gamma=" << gamma << ", p=" << p << "\n";
    ...
    cout << "Results written to tune_frozenlake.csv\n";
    return 0;
}
