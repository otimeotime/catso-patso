...
namespace {
    ...
    struct AutonomousVehicleMetrics {
        double cvar_regret;
    ...
    class AutonomousVehicleCvarOracle {
        private:
            shared_ptr<const mcts::exp::AutonomousVehicleEnv> env;
            double tau;
        ...
        public:
            AutonomousVehicleCvarOracle(shared_ptr<const mcts::exp::AutonomousVehicleEnv> env, double tau) :
                env(move(env)),
                tau(tau)
            {
            }
    ...
    static AutonomousVehicleMetrics evaluate_autonomous_vehicle_metrics(
        shared_ptr<const mcts::exp::AutonomousVehicleEnv> env,
        shared_ptr<const mcts::MctsDNode> root,
        ...
        double optimal_action_regret_threshold)
    {
        ...
        AutonomousVehicleMetrics metrics{numeric_limits<double>::quiet_NaN(), 0.0};
    ...
    static vector<Candidate> build_candidates(double default_eval_tau) {
        ...
        for (int n_atoms : {25, 51, 100}) {
            for (double optimism : {0.5, 1.0, 2.0, 4.0}) {
                for (double tau : tau_values) {
                    string cfg = "atoms=" + to_string(n_atoms)
                        + ",optimism=" + to_string(optimism)
                        + ",p=" + to_string(p_mean)
                        + ",tau=" + to_string(tau);
                    cands.push_back({"CATSO", cfg, tau, [n_atoms, optimism, tau](auto env, auto init_state, int max_depth, int seed) {
            ...
            for (double optimism : {0.5, 1.0, 2.0, 4.0}) {
                for (double tau : tau_values) {
                    string cfg = "particles=" + to_string(max_particles)
                        ...
                        + ",p=" + to_string(p_mean)
                        + ",tau=" + to_string(tau);
                    cands.push_back({"PATSO", cfg, tau, [max_particles, optimism, tau](auto env, auto init_state, int max_depth, int seed) {
...
int main(int argc, char** argv) {
    ...
    const array<double, 2> edge_probs = mcts::exp::AutonomousVehicleEnv::default_edge_probs();
    ...
    map<double, AutonomousVehicleCvarOracle> oracle_by_tau;
    for (const auto& cand : candidates) {
        oracle_by_tau.try_emplace(cand.eval_tau, env, cand.eval_tau);
    }
    const auto& base_root_solution = oracle_by_tau.at(cvar_tau).solve_state(env->get_initial_state());

    ofstream out("tune_autonomous_vehicle.csv", ios::out | ios::trunc);
    out << setprecision(17);
    ...
    for (size_t cand_idx = 0; cand_idx < candidates.size(); ++cand_idx) {
        ...
        for (int run = 0; run < runs; ++run) {
            ...
            out << "autonomous_vehicle," << cand.algo << "," << csv_escape(cand.config) << "," << cand.eval_tau << "," << run
    ...
    cout << "Results written to tune_autonomous_vehicle.csv\n";
    return 0;
}
