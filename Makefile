...
# Parameter tuning main for Autonomous Vehicle
...
# Risky Shortcut Gridworld single-config evaluator
EVAL_RISKY_SHORTCUT_GRIDWORLD_SOURCES = src/exp/eval_risky_shortcut_gridworld.cpp
EVAL_RISKY_SHORTCUT_GRIDWORLD_OBJECTS = $(patsubst src/%.cpp, bin/src/%.o, $(EVAL_RISKY_SHORTCUT_GRIDWORLD_SOURCES))
...
DEPFILES = \
	...
	$(EVAL_RISKY_SHORTCUT_GRIDWORLD_OBJECTS:.o=.d) \
...
# Build risky shortcut gridworld eval object files rule
$(EVAL_RISKY_SHORTCUT_GRIDWORLD_OBJECTS): $$(patsubst $(BIN_DIR)/%.o, %.cpp, $$@)
	@mkdir -p $(@D)
	$(CXX) $(CPPFLAGS) $(DEPFLAGS) -c -o $@ $<
...
# Build Risky Shortcut Gridworld single-config evaluator
$(TARGET_MCTS_EVAL_RISKY_SHORTCUT_GRIDWORLD): $(COMMON_OBJECTS) $(ENV_OBJECTS) $(EVAL_RISKY_SHORTCUT_GRIDWORLD_OBJECTS)
	$(CXX) $(CPPFLAGS) -o $@ $^ $(LDFLAGS)
...
clean:
	...
	@[ -f $(TARGET_MCTS_EVAL_RISKY_SHORTCUT_GRIDWORLD) ] && @rm $(TARGET_MCTS_EVAL_RISKY_SHORTCUT_GRIDWORLD) > /dev/null 2> /dev/null || :
	...
	@[ -f $(TARGET_MCTS_TUNE_AUTONOMOUS_VEHICLE) ] && @rm $(TARGET_MCTS_TUNE_AUTONOMOUS_VEHICLE) > /dev/null 2> /dev/null || :
